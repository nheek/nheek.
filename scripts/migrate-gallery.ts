...
async function migrateGallery() {
  ...
  if (existingCount.count > 0) {
    console.log(
      `Gallery already has ${existingCount.count} images. Skipping migration.`,
    );
    console.log("If you want to re-migrate, delete existing images first.");
    db.close();
    return;
  }
  ...
  try {
    migrateImages();
    console.log(
      `✅ Successfully migrated ${oldImages.length} gallery images to database`,
    );
  } catch (error) {
