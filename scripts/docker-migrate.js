...
async function migrate() {
  ...
  try {
    ...
    try {
      ...
      if (!columnNames.includes("codename")) {
        ...
        const updateStmt = db.prepare(
          "UPDATE albums SET codename = ? WHERE id = ?",
        );
    ...
    try {
      ...
      if (
        !columnNames.includes("track_order") &&
        columnNames.includes("track_number")
      ) {
        console.log(
          "  Migrating track_number to track_order in songs table...",
        );
        db.exec(`
        ...
        db.exec(
          "ALTER TABLE songs ADD COLUMN track_order INTEGER NOT NULL DEFAULT 1",
        );
      ...
      if (!columnNames.includes("codename")) {
        ...
        const updateStmt = db.prepare(
          "UPDATE songs SET codename = ? WHERE id = ?",
        );
    ...
    const existingAlbums = db
      .prepare("SELECT COUNT(*) as count FROM albums")
      .get();
    if (existingAlbums.count > 0) {
      console.log(
        "⏭️  Database already contains data. Skipping data migration.",
      );
      console.log(
        "� To force re-import, delete the database file or set FORCE_MIGRATION=true",
      );
      ...
      console.log(
        "⚠️  FORCE_MIGRATION=true - Re-importing data (this will overwrite existing data!)",
      );
    }
    ...
    if (!fs.existsSync(projectsPath)) {
      ...
      const categoryMap = {
        ...
        contributions: {
          name: "Open Source",
          slug: "contributions",
          description: "Open source contributions and collaborations",
        },
        static: {
          name: "Static Sites",
          slug: "static",
      ...
      console.log(
        `✅ Migrated ${categoriesCount} categories and ${projectsCount} projects`,
      );
    }
    ...
    console.log(
      `✅ Updated ${albumsLinksUpdated} albums and ${songsLinksUpdated} songs with custom links`,
    );
