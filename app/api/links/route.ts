...
export async function GET() {
  ...
  const links = db
    .prepare(
      "SELECT id, name, url, desc, color, display_order FROM links ORDER BY display_order ASC, id ASC",
    )
    .all();
...
export async function PUT(req: NextRequest) {
  ...
  const stmt = db.prepare(
    "UPDATE links SET name = ?, url = ?, desc = ?, color = ?, display_order = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
  );
