  ...
  try {
    ...
    if (existingCount.count > 0) {
      return NextResponse.json(
        {
          error: "Gallery already has images",
          message: `Gallery already contains ${existingCount.count} images. Delete existing images first if you want to re-migrate.`,
    ...
    const migrateImages = db.transaction(() => {
      oldImages.forEach((image, index) => {
        insertImage.run(image.src, image.alt, index + 1);
      });
    });
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully migrated ${oldImages.length} gallery images to database`,
      count: oldImages.length,
    });
  } catch (error) {
    console.error("Error migrating gallery:", error);
    return NextResponse.json(
      { error: "Failed to migrate gallery images" },
      { status: 500 },
    );
  }
}
